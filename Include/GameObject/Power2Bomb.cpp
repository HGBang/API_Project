...
CPower2Bomb::CPower2Bomb() :
	m_EffectType(EEffect_Type::Duration),
	m_Duration(0.f),
	m_Time(0.f)

{
	SetTypeID<CPower2Bomb>();
}

CPower2Bomb::CPower2Bomb(const CPower2Bomb& Obj) :
	CGameObject(Obj)
{
}
...
bool CPower2Bomb::Init()
{
	...
	SetCurrentAnimationEndFunction<CPower2Bomb>(this, &CPower2Bomb::AnimationEnd);
	...
	Box->SetCollisionProfile("Power2Bomb");
	Box->SetCollisionBeginFunction<CPower2Bomb>(this, &CPower2Bomb::CollisionBegin);
	Box->SetCollisionEndFunction<CPower2Bomb>(this, &CPower2Bomb::CollisionEnd);
...
void CPower2Bomb::Update(float DeltaTime)
{
	...
	if (m_EffectType == EEffect_Type::Duration)
	{
		...
		if (m_Duration <= m_Time)
		{
			...
			Effect1->SetPos(m_Pos.x + 5.f, m_Pos.y);
			Effect1->AddAnimation("BoomDownPower2", false, 0.4f);
			Effect1->SetPhysicsSimulate(true);
			Effect1->SetSideWallCheck(true);
			Effect1->SetBomb(true);
			Effect1->SetAngle(90.f);
			Effect1->SetMoveSpeed(500.f);
			Effect1->SetDistance(70.f);

			CPower1Effect* Effect2 = m_Scene->CreateObject<CPower1Effect>("BombEffect");
			Effect2->SetPivot(0.f, 0.f);
			Effect2->SetDamage(10.f);
			Effect2->SetPos(m_Pos.x + 5.f, m_Pos.y - 100.f);
			Effect2->AddAnimation("BoomUpPower2", false, 0.3f);
			...
			Effect3->SetPos(m_Pos.x , m_Pos.y + 5.5f);
			Effect3->AddAnimation("BoomRightPower2", false, 0.3f);
...
void CPower2Bomb::PostUpdate(float DeltaTime)
{
	CGameObject::PostUpdate(DeltaTime);
}

void CPower2Bomb::Render(HDC hDC, float DeltaTime)
{
	CGameObject::Render(hDC, DeltaTime);

}

void CPower2Bomb::CollisionBegin(CCollider* Src, CCollider* Dest)
{
...
void CPower2Bomb::CollisionEnd(CCollider* Src, CCollider* Dest)
{
	Dest->GetOwner()->SetBombOK(true);
}

void CPower2Bomb::AnimationEnd()
{
