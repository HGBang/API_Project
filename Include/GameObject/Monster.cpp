...
bool CMonster::Init()
{
	...
	CreateAnimation();
	AddAnimation("MonsterRight");
	AddAnimation("MonsterLeft");
	AddAnimation("MonsterUp");
	AddAnimation("MonsterDown");
	...
	m_NameBar->GetWidget<CText>()->SetText(TEXT("Monster"));
...
void CMonster::Update(float DeltaTime)
{
	...
	if (m_Move.x < 0.f)
		ChangeAnimation("MonsterLeft");

	if (m_Move.x > 0.f)
		ChangeAnimation("MonsterRight");

	if (m_Move.y > 0.f)
		ChangeAnimation("MonsterDown");

	if (m_Move.y < 0.f)
		ChangeAnimation("MonsterUp");
...
float CMonster::InflictDamage(float Damage)
{
	...
	if (m_HP <= 0)
	{
		...
		CMonsterDieEffect* Effect = m_Scene->CreateObject<CMonsterDieEffect>("MonsterDie1");
		Effect->SetPivot(0.5f, 0.5f);
		Effect->SetPos(m_Pos.x, m_Pos.y - 25.f);
		Effect->AddAnimation("MonsterDie1", false, 2.5f);
